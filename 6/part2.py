...
class MemoryHandler:
    ...
    def findCyclesToDuplicate(self):
        if self.duplicateIndex == -1:
            return -1
        lastIndex = len(self.previousStates) - 1
        return lastIndex - self.duplicateIndex + 1
    ...
    def hasDuplicateState(self):
        ...
        hasDuplicate = False
        for idx, state in enumerate(self.previousStates):
            isEqual = True
            for idx2, value in enumerate(state):
                if self.currentState[idx2] != state[idx2]:
                    isEqual = False
                    break
            if isEqual:
                hasDuplicate = True
...
def main():
    ...
    memoryHandler.run()
    print(memoryHandler.findCyclesToDuplicate())
